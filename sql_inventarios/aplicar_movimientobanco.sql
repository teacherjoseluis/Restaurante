...
BEGIN
    ...
    EXECUTE 'SELECT "Documento"."Id_ClaveFolio", "Documento"."Id_Usuario", "Documento"."Id_ConceptoDocumento", "Detalle_Documento"."Id_PersonaFiscal", "Detalle_Documento"."Id_CuentaBancaria1", "Detalle_Documento"."Id_CuentaBancaria2", "Detalle_Documento"."Subtotal" FROM "Documento" INNER JOIN "Detalle_Documento" ON "Documento"."ID" = "Detalle_Documento"."Id_Documento" WHERE "ID" =' || id_documento INTO id_clavefolio, id_usuario, id_concepto, id_personafiscal, id_cuentabancaria1, id_cuentabancaria2, subtotal;
	...
	FOR detalledocid IN detalledoc LOOP
	  ...
	  -- Actualmente la funcion crear documento setea el id_documentoorigen a 0. Cambiar esto
          EXECUTE 'SELECT crear_documento('||id_sucursal||',''Flujo_Banco'','||id_usuario||', '||id_documento||', '||id_concepto||')' INTO id_docban;
