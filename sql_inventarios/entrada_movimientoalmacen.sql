...
BEGIN
    -- 1. Se consulta del documento dado como parametro el Concepto - Id_Concepto
    EXECUTE 'SELECT "Documento"."Id_ConceptoDocumento", "Documento"."Id_ClaveFolio", "Documento"."Id_Usuario", "Detalle_Documento"."Id_UbicacionFisica1", "Detalle_Documento"."Id_UbicacionFisica2" FROM "Documento" INNER JOIN "Detalle_Documento" ON "Documento"."ID" = "Detalle_Documento"."Id_Documento" WHERE "Documento"."ID" =' || id_documento INTO id_concepto, id_clavefolio, id_usuario, id_ubicacionfisica1, id_ubicacionfisica2;
    ...
    -- Por default la ubicacion fisica 1 se considera el almacen de entrada
	...
	raise info '%', id_docalm;
	  ...
	  EXECUTE 'INSERT INTO "RegMaestro_UbicacionFisica"("Id_RegistroMaestro", "Id_UbicacionFisica", "Existencias") SELECT '||id_registromaestro||', '|| id_ubicacionfisica||', '||cantidad||' FROM "RegMaestro_UbicacionFisica" WHERE Id_UbicacionFisica='||id_ubicacionfisica||' WHERE NOT EXISTS (SELECT 1 FROM "RegMaestro_UbicacionFisica" WHERE Id_UbicacionFisica='||id_ubicacionfisica||' LIMIT 1 )';
