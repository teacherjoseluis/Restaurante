-- La recepcion de mercancias deber[a considerar la conversion entre presentaciones y unidades de medida
...
EXECUTE 'SELECT "ID" FROM "Clave_Folio" WHERE "NombreDocumento"= '||tipodocumento||' AND "Id_ClienteSistema"= '|| id_ClienteSistema INTO idclavefolio;
...
EXECUTE 'SELECT "ID" FROM "Documento_Asiento" WHERE "Id_ClaveFolio"='||id_clavefolio||' AND "Id_ConceptoDocumento"='||id_conceptodocumento||' AND "Id_Movimiento"='|| id_movimientodocumento INTO id_asiento;
...
RAISE INFO 'Según los valores con los que este documento se ha creado, la cuenta de cargo, %, con saldo %, modificará su valor a % y la cuenta de cargo, %, con saldo %, modificará su valor a %',nombre_cuentacargo,saldocargo,nuevo_saldocargo,nombre_cuentaabono,saldoabono,nuevo_saldoabono;
-------------------------------/*/*/*/*/*/*/*/*/*/----------------------------------------------------------------------------------------
EXECUTE 'SELECT LOCALTIMESTAMP' INTO fecha_hora;
EXECUTE 'INSERT INTO Documento ("Fecha/Hora", "Id_ClaveFolio", "Id_Usuario", "Monto", "Id_DocumentoOrigen", "Id_ConceptoDocumento", "FolioInterno", "Estatus", "Id_DocumentoMovimiento", "FolioDocumento") VALUES (''' || fecha_hora ||''', '||idclavefolio||', '||id_usuario||', '||monto||', '||id_documentoorigen||', '||id_conceptodocumento||', '||foliointerno||', ''N'', '||id_documentomovimiento||', '||folio||')';
